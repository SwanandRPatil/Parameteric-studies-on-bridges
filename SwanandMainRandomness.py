...
# response = np.ones((len(parameter1List), len(parameter2List)))
numSamples = 10

response = np.ones([(len(parameter2List))])
...
for i in range(len(parameter2List)):
    ...
    # Express damping in terms of the frequency, referred from the Der Kiureghian, 1991 report
    # zeta_g = (parameter2List[i] * 2.0 * 3.14) / 25.0

    # Damping, for medium and firm soils remains constant at 0.6, referred from Deodatis 1997 paper
    zeta_g = 0.60
    ...
    # Ground motion
    import SwanandGM2
    SwanandGM2.groundMotion(delta_t_gm,
                            duration_gm,
                            zeta_g,
                            parameter2List[i],
                            parameter3List[0],
    ...
    print("Response array, before:", response)

    # Finite element model
    import SwanandFEM4
    response[i] = SwanandFEM4.finiteElementModel(delta_t_gm,
                                                 delta_t_analysis,
                                                 duration_analysis,
                                                 parameter4List[0],
                                                 parameter5List[0],
...
disp1 = []
count = 0
filePath4 = r'C:\Users\swan0075\Dropbox\Swanand-Terje\Python Parametric Study\MSC-Concrete_1\plotTopNode1.out'
f = open(filePath4, "r")
lines = f.readlines()
for oneline in lines:
    splitline = oneline.split()
...
for oneline in lines:
    splitline = oneline.split()
    fxdBrgForce.append(float(splitline[0]))
    count += 1
...
for oneline in lines:
    splitline = oneline.split()
    fxdBrgDisp.append(float(splitline[0]))
    count += 1
